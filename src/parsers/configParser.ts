 ...
 *
 *  Author: Joss Moffatt josshmoffatt@gmail.com
 * 
 *--------------------------------------------------------------------------------------------*/
...
export class ConfigurationParser {
    ...
    public parse(): string {
        ...
        for (let section of this._sections) {
            let section_config = workspace_config.inspect<Configuration>(section);
            ...
            this.parseSection(section_changed_values, section)
        }
    ...
    private addNumericFlag(value: number, defaultValue: number, flag: string, errorMessage?: string): void {
        if(value === defaultValue)
            return;
        let condition = value > 0 && Number.isInteger(value)
        if(errorMessage === undefined)
            this.addFlag(condition, flag);
        else
    ...
    private addBooleanFlag(value: boolean, defaultValue: boolean, flag: string, errorMessage?: string): void {
        let condition = value === defaultValue;
        if(condition)
            return;
        if(errorMessage === undefined)
            this.addFlag(true, flag);
        else
            this.addFlag(true, flag, errorMessage);
    }
